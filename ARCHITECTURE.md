...
The repository does not force lockstep SDK versions. Each package will use SemVer, with a separate pinned spec commit recorded in generated metadata. Release tooling will validate and dry-run tags/artifacts before publication; actual publishing requires explicit authorization. A third language should consume the same snapshot and fixtures, generate its wire types into an isolated directory, then implement its native runtime and parity tests. Detailed instructions will follow the two concrete SDK implementations.
