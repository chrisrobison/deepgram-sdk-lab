...
An early Voice Agent client is available through `deepgram.agent.connect(configuration:)`. It waits for `Welcome`, sends `Settings`, and waits for `SettingsApplied` before returning a stream. The default configuration uses Deepgram Listen/Speak and an OpenAI think provider; `additionalAgentFields` permits provider-specific overrides. Voice Agent remains marked planned in the parity table until its contract tests and live handshake are verified.
